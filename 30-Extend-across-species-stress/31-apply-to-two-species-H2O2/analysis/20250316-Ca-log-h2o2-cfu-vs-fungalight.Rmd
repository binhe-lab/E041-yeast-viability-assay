---
title: "Apply FungaLight to _C. albicans_ treated with H2O2, compare with CFU"
...
This notebook will focus on the _C. albicans_ data.
...
data.path = "../input/20240318-23-h2o2-log-Ca-FGL/"
...
treatment.levels <- c("0", "0.6", "6", "1000")
...
pData(dat) <- sample
write_tsv(pData(dat), file = "../input/20240318-log-h2o2-ca-sample.tsv")
```
...
for(date in unique(pData(gs)$Date)){
  p <- ggcyto(gs[pData(gs)$Date == date], aes(x = BL1.H, y = BL3.H), subset = "singlet") + 
    geom_hex(bins = 128) + 
    geom_gate(live.gate.ca) + 
    geom_stats(location = "gate", adjust = c(0.03, 2), digits = 1, ) + 
...
```{r}
gs_pop_add(gs, live.gate.ca, parent = "singlet", name = "live")
recompute(gs)
```
...
```{r}
gs_pop_add(gs, inter.gate.ca, parent = "singlet", name = "intermediate")
recompute(gs)
```
...
p <- ggcyto(gs[with(pData(gs), Date == "031824" & Dye == "Both")], 
            aes(x = BL1.H, y = BL3.H), subset = "singlet") + 
  geom_hex(bins = 128) + 
  geom_gate(live.gate.ca, colour = "gray20") + 
  geom_gate(inter.gate.ca, colour = "steelblue3") + 
...
ggsave("../output/20250316-Ca-FGL-representative-plot.png", 
       height = 2, width = 5.5)
```
...
p1
ggsave("../output/20250324-Ca-log-h2o2-cell-size-distribution-compare.png",
       width = 4, height = 4)
```
...
write_tsv(gated_stats, file = "../output/20250324-Ca-log-h2o2-gated-stats.tsv")
...
# export the data for paper
write_tsv(dye_based, file = "../output/20250324-Ca-h2o2-fgl-estimate.tsv")
```
```
PI <- final %>% 
  dplyr::filter(Dye == "PI") %>% 
...
```{r}
cfu_dye <- bind_rows(
  cfu_calc |> #dplyr::filter(Species == "Ca") |>
    mutate(Method = "CFU") |> 
...
FungaLight percent live and percent intermediate
```{r}
...
p.list <- list(
  labs(x = NULL, y = NULL),
  theme(
    plot.title = element_text(size = rel(0.7), hjust = 0.5,
                              face = 2, margin = margin(b = 2)),
    axis.text  = element_text(size = rel(0.7))
  )
)
plot_grid(
  p.cfu + p.list + theme(axis.text.x = element_blank()),
  NULL,
  p.dye.side_by_side + p.list + theme(legend.position = "none"),
  nrow = 3, rel_heights = c(1, 0.1, 1.12)
)
ggsave("../output/20250324-Ca-log-h2o2-CFU-FGL-compare.png",
       width = 2.5, height = 3)
```
...
p
ggsave("../output/20250324-Ca-perc-live-compare-three-methods.png",
       width = 4, height = 2.5)
```
