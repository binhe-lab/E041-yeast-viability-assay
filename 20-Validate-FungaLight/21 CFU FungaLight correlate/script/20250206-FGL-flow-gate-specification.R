...
# AmpB
# live cell gate
polygon <- matrix(c(0, 10^3.5, 10^3.5, 0, # BL1.H, green
                    10^2.5, 10^2.5, 0, 0),# BL3.H, red
                  ncol = 2)
colnames(polygon) <- c("BL1.H", "BL3.H")
live.gate.AmpB <- polygonGate(filterId = "live", .gate = polygon)
# intermediate gate
polygon <- matrix(c(10^3.5, 10^3.5, 10^5.5, 10^5.5,   # BL1.H, green
                    0, 10^2.5, 10^4.5, 0),# BL3.H, red
                  ncol = 2)
colnames(polygon) <- c("BL1.H", "BL3.H")
inter.gate.AmpB <- polygonGate(filterId = "inter", .gate = polygon)
# dead cell gate
polygon <- matrix(c(10^2.5, 10^3.5, 10^5.5, 10^2.5,
                    10^2.5, 10^2.5, 10^4.5, 10^4.5), ncol = 2)
colnames(polygon) <- c("BL1.H", "BL3.H")
dead.gate.AmpB <- polygonGate(filterId = "dead", .gate = polygon)
