...
```{r}
# plot the contour plot of BL1.H vs BL3.H and nBL1.H vs nBL3.H
ggplot(norm_data, aes(x = BL1.H, y = BL3.H)) +
  geom_density_2d() +  
  facet_grid(rows = vars(H2O2_conc), cols = vars(Treatment)) +
  theme_minimal() +
  labs(x = "BL1.H", y = "BL3.H") +
  scale_x_log10(limits = c(10^1, 10^5), 
                breaks = c(10^1, 10^2, 10^3, 10^4, 10^5)) +  # Adjusted to avoid 0
...
```{r}
# visual inspection: if the normalization change the conclusion.


```


### DO NOT USE THEM; has nothing to do with my cell distribution analysis
